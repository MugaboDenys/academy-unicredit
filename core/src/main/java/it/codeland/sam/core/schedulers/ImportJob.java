package it.codeland.sam.core.schedulers;
...
public class ImportJob implements Runnable {
    ...
    @ObjectClassDefinition(name = "csv-import-job", description = "Import CSV containing articles data")
    public static @interface Config {
        ...
        @AttributeDefinition(name = "Concurrent task", description = "Whether or not to schedule this task concurrently", type = AttributeType.STRING)
        boolean scheduler_concurrent() default false;

        @AttributeDefinition(name = "CSV file path", description = "CSV file path in DAM", type = AttributeType.STRING)
        String CSVFilePath() default "";

        @AttributeDefinition(name = "User", description = "User to access DAM", type = AttributeType.STRING)
        String User() default "";

        @AttributeDefinition(name = "Password", description = "Password for the given user", type = AttributeType.PASSWORD)
    ...
    @Override
    public void run() {
        String pagePath = "/content/homepage/magazine";
        String templatePath = "/apps/sam-academy/templates/article";
        Page articlePage;
        ...
        try {
            ...
            Page articles = pageManager.getPage("/content/homepage/magazine/");
            if (articles == null) {
            ...
            try {
                savedDateTime = new DateTime(
                        articlesContentNode.getProperty("lastModifiedImport").getValue().toString());
            } catch (Exception e) {
            ...
            while (rows.hasNext()) {
                ...
                if (row.length == FIELDS_MAX) {
                    ...
                    if (pageManager.getPage("/content/homepage/magazine/" + articleData.pageName + "/") != null) {
                    ...
                    articlePage = pageManager.create(pagePath, articleData.pageName, templatePath,
                            articleData.pageTitle);
            ...
            logger.debug("\n");
            logger.debug(
                    "#####################CSV IMPORT JOB REPORT(STATISTICS) ###########################################");
            logger.debug("****** Total new Articles found from CSV file: {}", statistics.processedRecords);
            logger.debug("****** Total Articles skipped: {}", statistics.skippedRecords);
            logger.debug("****** Total Articles with with errors: {}", statistics.errorRecords);
            logger.debug(
                    "####################################################################################################");
            logger.debug("\n");
        } catch (Exception e) {
    ...
    private Iterator<String[]> getRowsFromCsv(InputStream is) throws IOException {
        ...
        final Iterator<String[]> rows = csv.read(is, null);
        // Skip the first row
        if (rows.hasNext()) {
            rows.next();
        }
