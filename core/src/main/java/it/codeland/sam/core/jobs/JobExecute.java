package it.codeland.sam.core.jobs;
...
@Component( service= JobExecutor.class,
        ...
        property = {
                ...
                JobExecutor.PROPERTY_TOPICS + "=samuel/job/topic"
        })
public class JobExecute implements JobExecutor{
    ...
    public JobExecutionResult process(final Job job, JobExecutionContext context){
        ...
        try {
            ...
            Resource resource = resolver.getResource("/content/dam/sam-academy/articles.csv");
            if(resource == null){
           ...
           int i =0;
            ...
            Page magazinePage = pageManager.getPage("/content/homepage/magazine/");
            ...
            logger.info("Error occurs Here.............: {}", e.getMessage());
            e.printStackTrace();
        }

        logger.info("I CAN REACH THIS FAR {} " ,test);
        return test;
    }
    ...
    private Iterator<String[]> getRowsFromCsv(InputStream inputStram) throws IOException {
        ...
        csv.setFieldSeparatorRead(',');
         final Iterator<String[]> rows = csv.read(inputStram, null);
        // Skip the first row
        if (rows.hasNext()) {
            rows.next();
        }
